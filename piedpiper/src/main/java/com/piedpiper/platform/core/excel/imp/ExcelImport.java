...
public class ExcelImport extends AbastractExcelImport {
	...
	protected void doGenerateErrorExcel() throws Exception {
	...
	protected void doValidateExcelTemplet() throws Exception {
		rs = wb.getSheetAt(0);
		this.rowTotal = rs.getLastRowNum();
		if (this.rowTotal < this.importTypeBeginRow + 1) {
			throw new RuntimeException(
					"指定的导入开始行:(" + (this.importTypeBeginRow + 1) + ")大于导入Excel中所含有的行数(" + this.rowTotal + ")行!");
		}
