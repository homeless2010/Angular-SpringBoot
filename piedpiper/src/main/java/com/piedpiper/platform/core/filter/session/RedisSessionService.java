 ...
 public class RedisSessionService
 {
     ...
     if (jedis != null) {
       try
       {
         if (jedis.get(key.getBytes()) == null) {
           ...
           jedis.set(key.getBytes(), SerializeUtil.serialize(session), "nx".getBytes(), "ex".getBytes(), sessionTimeout);
         } else {
   ...
   public void saveSession(String key, Map session, int sessionTimeout)
   {
     ...
     if (jedis != null) {
       try {
         jedis.set(key.getBytes(), SerializeUtil.serialize(session), "xx".getBytes(), "ex".getBytes(), sessionTimeout);
       } catch (Exception e) {
         pool.returnBrokenResource(jedis);
         e.printStackTrace();
       } finally {
