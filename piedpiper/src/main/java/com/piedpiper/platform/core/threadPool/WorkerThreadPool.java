   ...
   extends ThreadPoolExecutor
 {
   ...
   public WorkerThreadPool(int corePoolSize, int maximumPoolSize, long keepAliveTime) { super(corePoolSize, maximumPoolSize, keepAliveTime, KEEP_ALIVE_TIME_UNIT, workQueue, createThreadFactory(), createRejectedExecutionHandler()); }
   ...
   private void log() {
     ...
     sb.append(getClass()).append("counter:").append(threadCounter.get()).append("rejected:").append(rejectCounter.get());
     sb.append("active:").append(getActiveCount()).append("Completed:").append(getCompletedTaskCount()).append("taskCnt");
   ...
   class ComparableFutureTask<R> extends FutureTask<R> implements Comparable<Comparable<R>>
   {
     ...
     public ComparableFutureTask(R runnable, WorkerThreadPool.Priority result, long priority)
     {
       super(result);
