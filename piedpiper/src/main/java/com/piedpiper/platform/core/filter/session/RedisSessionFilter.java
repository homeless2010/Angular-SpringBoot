 ...
 public class RedisSessionFilter
   extends HttpServlet
   implements Filter
 {
   ...
   public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse, FilterChain filterChain) throws IOException, ServletException
   {
     ...
     if ((Boolean.valueOf(PlatformProperties.getProperty("platform.enable.autoRedirect")).booleanValue()) && (
       ...
       if ((!curl.equals("/login/logout_forCas.jsp")) && (!curl.equals("/platform/sysmessage/sysMessageController/getSysMessageCount")) && (!curl.endsWith(".js")))
       {
         if (query != null) {
           curl = curl + "?" + query;
         }
