...
public class SysMenuAPImpl implements SysMenuAPI {
	...
	public void reLoad() throws Exception {
		VALUE_BYCODE.clear();
		String url = RestClientConfig.getRestHost("sysmenu") + "/api/platform6/sysmenu/SysMenu/reLoad/v1";
	...
	public List<SysMenu> getSubSysMenus(String parentId) {
		return this.baseCacheManager.getAllFromCache("PLATFORM6_MENU_MENU_" + parentId, new TypeReference() {
   ...
   {
     (SysMenuTl)this.baseCacheManager.getObjectFromCache("PLATFORM6_MENUTLUNITID", sysMenuId + "_" + languageCode, new TypeReference() {});
   }

	public <T> T getSysMenusTlById(String sysMenuId, String languageCode, TypeReference<T> typeReference) {
		return (T) this.baseCacheManager.getObjectFromCache("PLATFORM6_MENUTLUNITID", sysMenuId + "_" + languageCode,
				typeReference);
	}

	public List<LayoutModel> getLayoutModelList(String layoutId)
   {
     (List)this.baseCacheManager.getObjectFromCache("PLATFORM6_PORTLETFILE", layoutId, new TypeReference() {});
   }

	public List<List<LayoutModel>> getAllLayoutModelList()
   {
     this.baseCacheManager.getAllFromCacheByOrder("PLATFORM6_PORTLETFILE", new TypeReference()new Comparator {}, new Comparator()
     {
	...
	public List<SysMenu> getSubSysMenus(String parentId, String appId) {
		this.baseCacheManager.getAllFromCache("PLATFORM6_MENU_MENU_" + parentId, new TypeReference() {
		}, appId);
	}

	public int getSubSysMenusSize(String parentId) {
		return this.baseCacheManager.countSizeByKey("PLATFORM6_MENU_MENU_" + parentId);
	}
   ...
   {
     (SysMenu)this.baseCacheManager.getObjectFromCache("PLATFORM6_MENU_CODE", code, new TypeReference() {}, appId);
   }
	...
	public List<Map<String, Object>> recurgetSysMenuInfoByParentId_old(String languageCode, String applicationId,
		...
		String url = RestClientConfig.getRestHost("sysmenu")
				+ "/api/platform6/sysmenu/SysMenu/recurgetSysMenuInfoByParentId/v1";
		ResponseMsg<List<Map<String, Object>>> responseMsg = RestClient.doPost(url, parameter, new GenericType() {
		});
		if (responseMsg.getRetCode().equals("200")) {
		...
		for (SysMenu sysmenu : listMap) {
			...
			if (!StringUtils.isBlank(id)) {
				...
				if ((null == childCount) || (childCount.longValue() <= 0L)) {
					...
					if (level > 0) {
						List<Map<String, Object>> cnodes = recurgetSysMenuInfoByParentId(languageCode, applicationId,
								(String) map.get("ID"), level, targetType, targetId);
	...
	public List<String> getAllocationPortalMenuIds(SecurityUser user) {
		String url = RestClientConfig.getRestHost("sysmenu")
				+ "/api/platform6/sysmenu/SysMenu/getAllocationPortalMenuIds/v1";
		ResponseMsg<List<String>> responseMsg = RestClient.doPost(url, user, new GenericType() {
		});
		if (responseMsg.getRetCode().equals("200")) {
			return (List) responseMsg.getResponseBody();
		}
	...
	public String getIconImageUrlMore(SysMenuVo sysMenuvo, String defaultIconImage, String showTabIcon,
		...
		try {
			if ((showDivIcon != null) && (Boolean.valueOf(showDivIcon).booleanValue())) {
				...
				sBuffer.append("<span class='icon' style='background-image:url(" + icons[0] + ");background-position:"
						+ icons[1]
						+ ";background-repeat:no-repeat no-repeat;display:block;float:left;with:16px;height:16px;'></span>");
				if (hasUrlFlag) {
				...
				if ((sysMenuvo.getHasChild() != null) && (sysMenuvo.getHasChild().booleanValue())) {
					sBuffer.append("<img src='" + firstDivIcon + "' border=0 style='margin-right:3px;'>");
				} else {
					sBuffer.append("<img src='" + secondDivIcon + "' border=0  style='margin-right:3px;'>");
				}
				if (hasUrlFlag) {
	...
	private Object getEachColElementForPortal(String defaultImage, String parentId, String showTabIcon,
		...
		List<List<SysMenuVo>> list = menu.getChildColumnMenu(parentId);
		StringBuilder sBuffer = new StringBuilder();
	...
	public String getIndexElementVlign(String currentLanguagecode, SecurityUser loginUser, SecurityMenu menu,
		...
		List<SysMenu> rootList = getSubSysMenus("-1", appId);
	...
	public String getIconImageUrl(SysMenuVo sysMenuvo, String defaultIconImage, String showTabIcon, String showDivIcon,
		...
		if ((showDivIcon != null) && (Boolean.valueOf(showDivIcon).booleanValue())) {
			...
			if ((sysMenuvo.getHasChild() != null) && (sysMenuvo.getHasChild().booleanValue())) {
				sBuffer.append("<img src='" + getProfileValueByCode("PLATFORM_V6_PMO_FIRST_TITLE_ICON", appid)
						+ "' border=0 style='margin-right: 3px;'>");
			} else {
				sBuffer.append("<img src='" + getProfileValueByCode("PLATFORM_V6_PMO_SECONDE_TITLE_ICON", appid)
						+ "' border=0 style='margin-right: 3px;'>");
			}
			if (hasUrlFlag) {
				sBuffer.append("<span style='line-height:20px;cursor:pointer;'>")
	...
	public String getDefaultSysFunction(String applicationId, String currentLanguage) {
		String url = RestClientConfig.getRestHost("sysmenu")
				+ "/api/platform6/sysmenu/SysMenu/getDefaultSysFunction/v1";
		Muti2Bean<String, String> bean = new Muti2Bean(applicationId, currentLanguage);
	...
	public String createTab4DefaultSysFunction(String sysMenuCode, String applicationId, String currentLanguageCode) {
		String url = RestClientConfig.getRestHost("sysmenu")
				+ "/api/platform6/sysmenu/SysMenu/createTab4DefaultSysFunction/v1";
		Muti3Bean<String, String, String> bean = new Muti3Bean(sysMenuCode, applicationId, currentLanguageCode);
		ResponseMsg<String> responseMsg = RestClient.doPost(url, bean, new GenericType() {
		});
		if (responseMsg.getRetCode().equals("200")) {
			return (String) responseMsg.getResponseBody();
		...
		if ((showSearchDialog != null) && (Boolean.valueOf(showSearchDialog).booleanValue())) {
			sBuffer.append("<div id='smartSearch' class=''>").append("")
					.append("<input type='text' class='search_subject' id='subjectSearch' name='subjectSearch' onfocus='onFocusEvent(this)' onblur='onBlurEvent(this)' value='"
							+ PlatformLocalesJSTL.getBundleValue("platform.search.onFocus.tip", locale)
	...
	public void reloadHomePageMenuCache() {
		VALUE_BYCODE.clear();
		String url = RestClientConfig.getRestHost("sysmenu")
				+ "/api/platform6/sysmenu/SysMenu/reloadHomePageMenuCache/v1";
	...
	public List<SysMenuVo> getAllSubObjectInPermisson(String appId, String loginName, String userid,
		...
		Muti6Bean<String, String, String, String, Boolean, Set> args = new Muti6Bean(appId, loginName, userid,
				lauguageCode, Boolean.valueOf(flag), permissions);
		String url = RestClientConfig.getRestHost("sysmenu")
				+ "/api/platform6/sysmenu/SysMenu/getAllSubObjectInPermisson/v1";
		ResponseMsg<List<SysMenuVo>> responseMsg = RestClient.doPost(url, args, new GenericType() {
		});
		if (responseMsg.getRetCode().equals("200")) {
			return (List) responseMsg.getResponseBody();
		}
	...
	public int checkSysmeueCode(String code, String menuId) {
		...
		Muti3Bean<String, String, String> checkparam = new Muti3Bean();
		checkparam.setDto1(RestClientConfig.systemid);
		checkparam.setDto2(code);
		checkparam.setDto3(menuId);
		ResponseMsg<Integer> response = RestClient.doPost(checkCode, checkparam, new GenericType() {
		});
		if (response.getRetCode().equals("200")) {
			return ((Integer) response.getResponseBody()).intValue();
		}
		log.error("url:" + checkCode + ",error:" + response.getErrorDesc());
		return -3;
	}

	public String insertMenu(SysMenu sysMenu) {
		String url = RestClientConfig.getRestHost("sysmenu") + "/api/platform6/sysmenu/SysMenu/insertmenu/zh_CN/v1";
		ResponseMsg<String> responseMsg = RestClient.doPost(url, sysMenu, new GenericType() {
		});
		if (responseMsg.getRetCode().equals("200")) {
			return (String) responseMsg.getResponseBody();
		}
	...
	public Integer insertMenus(List<SysMenu> sysMenu) {
		String url = RestClientConfig.getRestHost("sysmenu") + "/api/platform6/sysmenu/SysMenu/insertmenus/zh_CN/v1";
		ResponseMsg<Integer> responseMsg = RestClient.doPost(url, sysMenu, new GenericType() {
		});
		if (responseMsg.getRetCode().equals("200")) {
	...
	public void updateMenu(SysMenu sysMenu) {
		String url = RestClientConfig.getRestHost("sysmenu") + "/api/platform6/sysmenu/SysMenu/updatemenu/zh_CN/v1";
		ResponseMsg<Void> responseMsg = RestClient.doPost(url, sysMenu, new GenericType() {
		});
		if (!responseMsg.getRetCode().equals("200")) {
	...
	public void deleteMenu(String id) {
		String url = RestClientConfig.getRestHost("sysmenu") + "/api/platform6/sysmenu/SysMenu/deletemenu/v1";
		ResponseMsg<Void> responseMsg = RestClient.doPost(url, new String[] { id }, new GenericType() {
		});
		if (!responseMsg.getRetCode().equals("200")) {
	...
	private String getChildrenElementVlign(String parentId, String appId, SecurityMenu menu, String beginClickEvent,
		...
		List<SysMenuVo> list = menu.getMenusByParentId(parentId);
	...
	public boolean checkSysMenuUrl(String url) {
		String resturl = RestClientConfig.getRestHost("sysmenu") + "/api/platform6/sysmenu/SysMenu/checkSysMenuUrl/v1";
		ResponseMsg<Boolean> responseMsg = RestClient.doPost(resturl, url, new GenericType() {
		});
		if (responseMsg.getRetCode().equals("200")) {
			return ((Boolean) responseMsg.getResponseBody()).booleanValue();
		}
		log.error("url:" + resturl + ",error:" + responseMsg.getErrorDesc());
		return true;
	}
			...
			for (int i = 0; i < list.size(); i++) {
				...
				if ("1".equals(sysMenuVo.getType())) {
					...
					if (sysMenuVo.getHasChild().booleanValue()) {
						...
						sBuffer.append(getLeftTreeMenuChildrens(sysMenuVo.getId(), appId, menu, beginClickEvent,
								upDonote, seperator, afterClickEvent, returnValue, defaultImage, showTabIcon,
								iconTipsFlag, showDivIcon, index + 1));
	...
	public String getLeftTreeIconImageUrl(SysMenuVo sysMenuvo, String defaultIconImage, String showTabIcon,
		...
		try {
			if ((showDivIcon != null) && (Boolean.valueOf(showDivIcon).booleanValue())) {
				sBuffer.append("<span class='icon' style='background-position: "
						+ getLeftTreeIconPara(sysMenuvo.getImage(), defaultIconImage, showTabIcon) + ";'></span>");
				if (hasUrlFlag) {
