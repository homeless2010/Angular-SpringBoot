 ...
 public class SecurityUtil
 {
   ...
   public static String buildSimpleUrl(String oUrl) { String url = oUrl;
     url = parseUrl(url);
     if (url.startsWith("/")) {
   ...
   private static void refalshShiroCaceh() { SecurityUtils.getSubject().isPermitted("shuaxinquanxia"); }
     ...
     try
     {
       ...
       if (defaultMenus != null)
       {
         if (staticInt < refushInt) {
           staticInt = refushInt;
           defaultMenus = ((BaseCacheManager)SpringFactory.getBean("baseCacheManager")).getAllFromCache("avicit.menu.default_" + RestClientConfig.systemid, new TypeReference() {});
         }
   ...
   public static int getOnlineUserNum(String applictionId)
   {
     ...
     Set<String> keys = ((Jedis)jedis.getAllShards().iterator().next()).keys("*" + applictionId + "_SESSION");
     int userNum = 0;
     for (String key : keys) {
   ...
   public static void checkModule(String simpleName)
   {
     ...
     if (("true".equals(checkFlag.toLowerCase())) && 
       ...
       try {
         ...
         ContextHolder.getResponse().sendRedirect(ViewUtil.getRequestPath(ContextHolder.getRequest()) + "com.piedpiper/modules/system/sanyuanlogin/sanyuanlogin.jsp");
       } catch (IOException e) {
   ...
   public static void deleteCache(String key)
   {
     JedisSentinelPool jedisSentinelPool = (JedisSentinelPool)SpringFactory.getBean("jedisSentinelPool");
     ShardedJedis jedis = (ShardedJedis)jedisSentinelPool.getResource();
     Set<String> keys = ((Jedis)jedis.getAllShards().iterator().next()).keys("*" + key + "*");
   ...
   public static boolean checkUrlInPermiss(String url)
   {
     ...
     if (Boolean.valueOf(cassFull).booleanValue()) {
       List<SysResource> listResource = sysResourceAPI.getAllList();
   ...
   public static String matchingUppercase(String loginName) { String matchUpperCase = PlatformProperties.getProperty("avicit.security.matchUpperCase");
