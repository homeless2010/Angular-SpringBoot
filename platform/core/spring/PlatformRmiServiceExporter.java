...
public class PlatformRmiServiceExporter extends RmiBasedExporter implements InitializingBean, DisposableBean {
	...
	protected Registry getRegistry(String registryHost, int registryPort, RMIClientSocketFactory clientSocketFactory,
		...
		if (registryHost != null) {
			if (this.logger.isInfoEnabled()) {
				this.logger
						.info("Looking for RMI registry at port '" + registryPort + "' of host [" + registryHost + "]");
			}
	...
	protected Registry getRegistry(int registryPort, RMIClientSocketFactory clientSocketFactory,
		...
		if (clientSocketFactory != null) {
			...
			if (this.logger.isInfoEnabled()) {
				this.logger
						.info("Looking for RMI registry at port '" + registryPort + "', using custom socket factory");
			}
	...
	public void destroy() throws RemoteException {
		if (this.logger.isInfoEnabled()) {
			this.logger.info("Unbinding RMI service '" + this.serviceName + "' from registry"
		...
		try {
			...
			if (this.logger.isWarnEnabled()) {
				this.logger.warn("RMI service '" + this.serviceName + "' is not bound to registry"
